...
export function createRemoveReactionTool(ctx: QueryToolContext) {
  return tool(
    "remove_reaction",
    "Remove an emoji reaction from a Slack message. Provide either a Slack message URL or channel_id + message_ts to identify the message.",
    {
    ...
    async (args) => {
      ...
      try {
        await ctx.slackClient.reactions.remove({
        ...
        return errorResult(`Failed to remove reaction: ${msg}`);
      }
    },
  );
}
