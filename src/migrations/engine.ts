...
export async function executeMigration(migration: Migration): Promise<void> {
  ...
  if (migration.static) {
    try {
      ...
      logger.info(
        `Migration "${migration.name}": static transform completed`
      );
    } catch (err) {
      ...
      if (migration.prompt) {
        ...
        throw new Error(
          `Migration "${migration.name}" static transform failed: ${errMsg}`
        );
      }
    }
  }
...
- If a file doesn't exist yet and needs to be created, write its full contents${
    ...
    for await (const message of query({
      ...
      if (message.type === "result") {
        if (message.subtype === "success") {
          ...
          throw new Error(
            `Migration "${migration.name}" failed: ${errMsg}`
          );
        }
      }
    }
    ...
    throw new Error(
      `Migration "${migration.name}" has neither static nor prompt defined`
    );
