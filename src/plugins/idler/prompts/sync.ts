 ...
 * recently-changed memory (gated by `scanMemory`), recompute priority — then rotates through the
 * EXTERNAL discovery sources one-per-fire. Memory is NOT part of the round-robin: it is maintained
...
${triageBlock}${config.sources.scanMemory ? "3" : "2"}. RECOMPUTE PRIORITY (every fire, regardless of memory triage): for every open unit call upsert_idea with the right kind + freshInput/blocked signals so priority reflects current state. A unit waiting on a human with no new activity is blocked (sinks); a fresh reply/comment past the cursor is freshInput (rises).
