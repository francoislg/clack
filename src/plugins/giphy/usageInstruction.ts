export const GIPHY_USAGE_INSTRUCTION = `# GIF usage (Giphy)

You have access to \`mcp__giphy__find_gif\` — a Giphy-backed search that returns real GIF URLs.
...
- **Attribution is required.** When your response includes a GIF, add a short \`Powered by GIPHY\` note somewhere in the message. A trailing \`context\` block is a clean way to do this.

## How to include a GIF

Include the GIF as a Block Kit **\`image\` block** inside the \`blocks\` array of your \`submit_response\` payload. Do NOT paste the GIF URL into the text body — Slack will not reliably unfurl \`media.giphy.com\` links.
...
\`\`\`json
{
  ...
  "blocks": [
    ...
    { "type": "image", "image_url": "https://media.giphy.com/....gif", "alt_text": "celebration gif" },
    { "type": "context", "elements": [ { "type": "mrkdwn", "text": "_Powered by GIPHY_" } ] }
  ]
}
\`\`\`
`;
