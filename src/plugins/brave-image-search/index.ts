...
export const braveImageSearchPlugin: ClackPlugin = async (sdk: ClackSdk) => {
  ...
  // `mcp__brave_image_search__find_image` and is available to trivia's scheduled-run prompt
  // without an `attach_integration` step. Loads even when BRAVE_API_KEY is unset — the tool then
  // returns `keyMissing` on every call so trivia's visual subflow silently moves on.
