...
export function createSetWorkHoursTool(sdk: ClackSdk) {
  return tool(
    "set_work_hours",
    "Set the work-hours window during which casual-talk may fire. start is inclusive (e.g. 9 = first fire at 9:00), end is exclusive (e.g. 16 = last fire at 15:45 on the 15-minute cadence). days are JS day-of-week numbers (0=Sun, 6=Sat). Triggers a soft restart on success.",
    {
    ...
    async (args) => {
      ...
      sdk.requestSoftRestart("casual-talk: work hours changed");
      return textResult({
        ok: true,
