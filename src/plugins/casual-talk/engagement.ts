...
export const ENGAGEMENT_CONTENT = [
  ...
  ``,
  `If you are POSTING, deliver by calling \`submit_response\` with a \`deliver_to\` array holding exactly ONE entry — \`{ channel, [thread_ts], attention_level, default_delivery_mode: "invisible", response: { blocks } }\` — where \`channel\` is the explicit destination, \`response.blocks\` is your message, and \`attention_level\` is chosen per the rule below. That single call both delivers and terminates the run. Do NOT also set \`skip_response\` when you are delivering: \`skip_response: true\` is ONLY for the no-post cases. Setting both at once is ignored — the post still goes — but don't do it.`,
  ``,
  ...
  ``,
  `Setting \`attention_level\` is MANDATORY on the entry — never omit it. This run is an INVISIBLE scheduled tick: you won't get another chance to engage this thread until the next random roll happens to land on it, which may be far in the future or never, and omitting the field leaves the thread fire-and-forget. Choose the value by WHOSE thread it is:`,
  `- **Opening a fresh top-level thread** (no \`thread_ts\` on the entry) — \`"high"\`. The thread is yours; a reply there is aimed at you, so answer it in real time.`,
  `- **Joining a thread that already exists** (\`thread_ts\` set) — \`"off"\` or \`"low"\`, never higher. You are a guest in someone else's conversation, and it will keep going without you. Use \`"low"\` when you'd want to answer someone who addresses you directly, \`"off"\` for a pure parting comment you expect no follow-up to. \`"low"\` still catches direct address and clear follow-ups — it only declines the side chatter that was never for you. Requesting anything above \`"low"\` here is wasted: the server clamps a joined thread to \`"low"\` regardless.`,
  ``,
