...
export function buildPrompt(args: BuildPromptArgs): string {
  ...
  return [
    ...
    `## Step 3 — Deliver`,
    ``,
    `Deliver by calling \`submit_response\` with a \`deliver_to\` array holding exactly ONE entry — \`{ channel, [thread_ts], attention_level: "high", default_delivery_mode: "invisible", response: { blocks } }\` — where \`channel\` is the explicit destination and \`response.blocks\` is your message. That single call both delivers and terminates the run. Do NOT also set \`skip_response\` when you are delivering: \`skip_response: true\` is ONLY for the no-post cases (a roll that wasn't 1, or the genuine-impossibility skip below). Setting both at once is ignored — the post still goes — but don't do it.`,
    ``,
