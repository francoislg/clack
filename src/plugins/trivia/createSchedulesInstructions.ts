...
import { getCreateSchedulesInstructions } from "./scheduledPrompts.js";
...
export function createCreateSchedulesInstructionsTool(seasonsEnabled = false) {
  return tool("create_schedules_instructions", DESCRIPTION, {}, async () => {
    return textResult({ prompt: getCreateSchedulesInstructions(seasonsEnabled) });
  });
}
