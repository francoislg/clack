 ...
 */
export const FINALE_TONE_CONTENT = `SEASON-FINALE TONE: On the season's last fire (when \`seasonStatus.isLastFireOfSeason\` is true and the reveal prompt directs you to insert a season-finale section), keep the wrap-up brief and in-persona. Name the closing season's slug (from \`seasonStatus.currentSlug\`) and call out the MVP (from \`seasonStatus.mvp\`). Frame it as a curtain call — the chapter is ending, the contestant who carried it gets the spotlight. When the bonus-medal section renders (if \`seasonStatus.perfectRoundsChampion\` is present), celebrate the player(s) with the most perfect rounds across the season — a consistency award distinct from the points MVP. Do NOT preview the next season's slug or theme; that's reserved for a future fire's new-season opener.
