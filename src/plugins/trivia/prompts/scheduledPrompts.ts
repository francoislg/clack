...
2. RENDER VIA \`submit_response\` USING THE GAME SHOW PRESENTER VOICE:
   ...
   - \`reveals.length > 1\`: MULTI-QUESTION layout (see below the single-question section). Use brief per-question verdicts, then a "Round Summary" section sourced from \`roundSummary.perPlayer\`. Skip the Round Summary block ONLY when \`roundSummary.perPlayer\` is empty (nobody answered this round).
   ...
   - **Round Summary section — GATED on \`roundSummary.perPlayer\` being non-empty.** \`roundSummary\` is ALWAYS in the payload (it is mode-independent — the reveal mode never suppresses the scoreboard), so render ONE \`section\` block titled "🏆 Round Summary" (or similar) whenever \`roundSummary.perPlayer\` has at least one entry: list each player IN ORDER as \`<@USERID>: <correct>/<totalQuestions>\` (or any in-persona phrasing). Prefix every entry whose \`roundMvp: true\` is set with \`🏆\` (e.g. "🏆 <@U123>: 3/3"). DO NOT recompute the counts — read them straight from \`roundSummary.perPlayer\`. DO NOT add players who aren't in \`perPlayer\` (the tool already filters out anyone who didn't answer this round). SKIP this Round Summary block ONLY when \`perPlayer\` is empty (nobody answered this round) — then the per-question verdicts and the cumulative leaderboard table below carry the closer on their own.
   ...
   - When \`roundSummary.perPlayer\` is empty (nobody answered this round): order columns by \`currentSeasonCorrect\` descending (or \`totalCorrect\` descending when seasons are off).
