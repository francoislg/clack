...
1. CALL \`compute_answers({ game: "{game}" })\` AND READ THE PAYLOAD:
   ...
   - \`reveals\`: array of reveal entries to render (length 0 = nothing pending, length 1 = today's reveal). Each entry has:
     ...
     - \`media\` (OPTIONAL — present ONLY on image-medium questions): \`{ title, attribution?, license? }\`. When present, append ONE \`context\` block to that question's reveal: \`{ type: "context", elements: [{ type: "mrkdwn", text: "📷 Image: <attribution> · <license>" }] }\`. Use the 📷 Unicode char (NEVER \`:camera:\`). Omit \` · <license>\` when \`license\` is absent or the literal \`"unknown"\` (an unknown license is noise, not attribution); omit the whole block when BOTH \`attribution\` and \`license\` are absent. The block goes immediately AFTER that question's verdict/answer section (in multi-question reveals, before the next question's divider); the cumulative leaderboard table stays last. This honors source license terms (CC-BY-SA requires attribution).
     ...
     - \`teamVoters\` (OPTIONAL — present ONLY in TEAMS MODE, see the TEAMS MODE section): the team-grouped projection of \`voters\`. \`{ correctTeams: Array<{ team, answerTexts? }>, correctFreeAgents: Voter[], incorrectTeams?, incorrectFreeAgents?, noAnswerTeams?: string[], noAnswerFreeAgents? }\`. A team is Correct when ≥1 member answered correctly, Incorrect when members answered but none correctly, NoAnswer when nobody on it answered. Members are ABSORBED into the team — the payload deliberately hides who did what inside a team. \`answerTexts\` (freeform, \`"yes"\` mode only) are the team's typed answers, UNATTRIBUTED.
   ...
   - \`teamStandings\` (OPTIONAL — its PRESENCE is the TEAMS MODE switch): \`{ scoring, teams: Array<{ name, thisRound, currentSeason, allTime? }>, freeAgents: Array<{ userId, displayName, thisRound, currentSeason, allTime }>, finaleIndividuals? }\`. \`teams\` is already ordered current-season points desc — that IS the table's team-column order. A team's \`allTime\` is present ONLY when the same team name existed in a prior season (absent → its All Time cell renders as \`"—"\`). See the TEAMS MODE section for the full rendering contract. Absent → individual play; ignore that section entirely.
   ...
   - \`seasonStatus\` (only present when \`trivia.seasons.enabled\` is true): \`{ currentSlug, isLastFireOfSeason, seasonClosed, hasPriorSeasons, mvp? }\`. This is REPORT-ONLY — \`compute_answers\` performs no rollover (\`seasonClosed\` is always \`false\` here). When \`isLastFireOfSeason\` is true you MUST call \`end_season({ game: "{game}" })\` in step 3 to perform the (idempotent) close; do NOT call \`upsert_season\`.
   - \`windDown\` (optional — only ever \`{ eligible: true }\`, absent otherwise): the seasonless analog of \`isLastFireOfSeason\`, reported when a seasonless game has finished its final board. REPORT-ONLY. When present you MUST call \`end_season({ game: "{game}" })\` in step 3 — the tool performs the wind-down itself.
...
3. ON THE ROUND'S FINAL FIRE ONLY, CALL \`end_season({ game: "{game}" })\`:

   Call this IF AND ONLY IF \`seasonStatus.isLastFireOfSeason === true\` OR \`windDown.eligible === true\`. On a season's last fire it stamps \`endedAt\` and resolves the successor (promotes a queued season, creates next month's continuation, or — when the game is configured to wind down — no successor at all); on a seasonless final board it winds the game down. It is idempotent — safe if the close already happened. When NEITHER gate holds, SKIP this call. NEVER pass \`force\` from the reveal flow — the tool self-verifies and on a genuine final fire closes without it; if it returns \`requiresConfirmation: true\`, that means this is NOT the final fire, so do NOT retry, just SKIP.
   READ THE RESULT: when it carries \`gameDisabled: true\`, the game just wound itself down for good — your closer in step 4 (the finale closer, or the normal \`context\` closer on a seasonless reveal) MUST be a series wrap: the chapter is closed, NO "see you next season", NO next-season preview, NO timing prediction. Otherwise the normal season-handoff closer applies.
...
4. RENDER VIA \`submit_response\` USING THE GAME SHOW PRESENTER VOICE:
   ...
   - \`section\` block (mrkdwn) — explain WHY the statement is true/false (boolean) or which choice was correct + why (choice). For FREEFORM, summarize the expected answer in one short factual sentence and (when \`answer.acceptableAnswers\` was populated) note that variants were accepted. **SUBSTANCE FLOOR — never merely negate.** A bare dismissal ("that wasn't it", "the claim was false") is not an explanation. Every WHY must name (a) the specific claim that was wrong (or the tempting-but-wrong choice), (b) the real/true fact, and (c) a one-clause reason it holds. E.g. NOT "False — nope!" but "False — the statement credits Edison with the lightbulb, but Joseph Swan demonstrated a working incandescent bulb first; Edison's win was commercial, not inventive." This floor applies to EVERY reveal, including ones everyone aced. Apply the REVEAL TONE from the \`trivia\` topic of your system instructions — tight, but the substance floor is non-negotiable.
   ...
   4. BONUS MEDAL (PERFECT ROUNDS CHAMPION) — ONE \`section\` (mrkdwn) rendered ONLY when \`seasonStatus.perfectRoundsChampion\` is present. Award a 🎖️ BONUS MEDAL to the player(s) in \`perfectRoundsChampion.userIds\` for having the most PERFECT ROUNDS (clean sweeps) across the entire season. Use \`count\` to show how many perfect rounds they achieved. Name each player following the SAME MENTION POLICY as the podium: real \`<@USERID>\` mentions when top-level \`tagPlayers\` is true, plain-text \`@displayName\` when false. List all tied players together on one line (e.g. "🎖️ *Most Perfect Rounds:* <@U_A> & <@U_B> — 4 clean sweeps"). This award is separate from and additional to the points podium. When \`seasonStatus.perfectRoundsChampion\` is ABSENT, render NOTHING for this step — no line, no placeholder.
   5. ALL-TIME TABLE — set the \`table\` parameter as a standalone all-time scoreboard ONLY when \`seasonStatus.hasPriorSeasons === true\` AND \`showAllTimeRow !== false\`. Introduce it with a one-line \`section\` ("And the all-time leaderboard:"). The table is a names-header row + an \`All Time\` row of \`String(totalPoints)\`, columns ordered by \`totalPoints\` descending, medaled by the DENSE-RANK MEDAL RULE below. When the gate fails — a single season (\`hasPriorSeasons\` false, where All Time would just duplicate the podium) OR \`showAllTimeRow\` is false (e.g. \`allTimeRow: "never"\`) — OMIT the \`table\` entirely.
   6. \`context\` CLOSER — branch on the step-3 \`end_season\` result. Normal handoff (no \`gameDisabled\`): e.g. "Thanks for playing — see you next season! 🎉"; do NOT preview the next season's slug even when the result carried \`newSeasonStarted\`. SERIES WRAP (\`gameDisabled: true\`): the game itself is over — close the chapter for good (e.g. "That's a wrap on the whole game — thanks for playing! 🎬"); NO "see you next season", NO next-season preview. Either way do NOT predict timing. \`end_season\` already stamped the closing season's \`endedAt\` before returning — do NOT call \`upsert_season\` as a follow-up.
   ...
   When \`teamStandings\` is present the game is playing in TEAMS: standings and shout-outs are by TEAM, individual team members are never named, and free agents (players in no team) keep playing as individuals. This section OVERRIDES the leaderboard-table column rules and the narrative naming rules above; everything else (layout branches, SUMMARY PLACEMENT, DENSE-RANK MEDAL RULE, MENTION POLICY for free agents) applies unchanged. When \`teamStandings\` is ABSENT, ignore this section — the individual contract above applies exactly as written.

   TABLE (replaces STEP 1–3 column selection; the row set is unchanged):
   - COLUMNS: one per team from \`teamStandings.teams\`, FIRST, in payload order (already current-season points desc); THEN one per free agent from \`teamStandings.freeAgents\`. NO column for any individual team member.
   - NAMES HEADER: team names and free-agent \`displayName\`s as plain text (team names are NEVER \`<@…>\` mentions).
   - \`This Round\` / \`Current Season\` cells: \`String(thisRound)\` / \`String(currentSeason)\` from the entry (team or free agent). The DENSE-RANK MEDAL RULE applies per row across ALL columns together — teams and free agents compete in the same row.
   - \`All Time\` row (same \`hasPriorSeasons\` + \`showAllTimeRow\` gate as individual play): a team WITH \`allTime\` renders \`String(allTime)\`; a team WITHOUT it renders the em-dash \`"—"\` (no medal) — its name has no prior-season history and you MUST NOT derive a value yourself. Free agents always render their \`allTime\`.
   - PERFECT-ROUND STARS are per-PLAYER flags — a team cell gets no ⭐; a free agent keeps theirs per the normal rule.
   - Do NOT recompute team numbers from \`voters\`, \`leaderboard\`, or \`roundSummary\` — \`teamStandings\` is the sole source for team cells.

   NARRATIVE: read voter shout-outs from each entry's \`teamVoters\`, NOT \`voters\`. Celebrate and console TEAMS by name ("Red takes it!", "Blue came up empty") and NEVER name a team member — a member name anywhere in a teams-mode reveal is a bug. Free agents (the \`*FreeAgents\` arrays) are still named individually per the MENTION POLICY. Freeform texts: quote a team's \`answerTexts\` UNATTRIBUTED ("Red's guesses included *Paris* and *Lyon*"); do not attribute a text to a person. The \`just-winners\` anonymous counts and the \`"no"\`-mode silence rules apply unchanged.

   SEASON FINALE in teams mode: build the podium from \`teamStandings.teams\` ranked by \`currentSeason\` (medal teams, plain-text names); free agents join the PARTICIPATION TAIL with their own pts (mention policy applies to them). Skip the individual \`seasonStatus.mvp\` shout-out unless that player is a free agent. The gated ALL-TIME table uses the team columns with the per-cell rule above. When \`teamStandings.finaleIndividuals\` is \`true\`, ADDITIONALLY append the classic INDIVIDUAL leaderboard table (from \`leaderboard\`, built per the individual contract) below the team finale — the one place member names may appear, and only inside that appended table.
