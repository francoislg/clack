...
4. RENDER VIA \`submit_response\` USING THE GAME SHOW PRESENTER VOICE:
   ...
   5. \`context\` CLOSER — branch on the step-3 \`end_season\` result. Normal handoff (no \`gameDisabled\`): e.g. "Thanks for playing — see you next season! 🎉"; do NOT preview the next season's slug even when the result carried \`newSeasonStarted\`. SERIES WRAP (\`gameDisabled: true\`): the game itself is over — close the chapter for good (e.g. "That's a wrap on the whole game — thanks for playing! 🎬"); NO "see you next season", NO next-season preview. Either way do NOT predict timing. \`end_season\` already stamped the closing season's \`endedAt\` before returning — do NOT call \`upsert_season\` as a follow-up.
