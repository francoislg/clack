...
describe("upsert_season — format argument", () => {
  ...
  it("create stores format verbatim and returns hasFormat: true", async () => {
    ...
    const result = await tool.handler(
      {
        ...
        format: {
          questions: [
            { label: "Q1" },
            { label: "Q2", categories: ["History"], questionTypes: { choice: 1 } },
          ],
        },
      },
      SESSION,
    ...
    assert.deepEqual(entry?.format?.questions[1].questionTypes, { boolean: 0, choice: 1 });
  });
    ...
    const result = await tool.handler(
      {
        ...
        categories: undefined,
        questionTypes: undefined,
        format: { questions: [{ questionTypes: { boolean: 0, choice: 0 } }] },
      },
      SESSION,
    );
    const parsed = parseToolResult(result);
