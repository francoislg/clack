...
- \`suggestedPromptMedium\`: \`"text"\` or \`"image"\` — picked INDEPENDENTLY from active promptMedium weights (slot → season → game → workspace → \`{ text: 1, image: 0 }\` default). \`"image"\` REQUIRES Claude to follow the VISUAL RESEARCH SUBFLOW: pick an available \`*_image_search__*\` MCP tool fitting the rolled category, call it, inspect the inline image, and save \`promptMedium: "image"\` + a \`media\` object. When NO \`*_image_search__*\` tool is installed (or no usable image is found within the retry budget), fall back to the text-medium path for the same \`answersFormat × questionType\`. \`categories.ideas\` is drawn from the same pool regardless of medium.
