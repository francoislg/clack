...
- Personal preferences: `sdk.registerPreferences({ schema, fields })` declares per-user toggle fields rendered into the Home Tab settings modal (labels resolve through the plugin's own `sdk.t` dictionary); `sdk.preferences.get(userId, schema)` reads back the user's slice. This is a USER-owned choice store — read-only for the plugin — distinct from `sdk.users.data(schema)`, which is the plugin's own read/write extension namespace.
