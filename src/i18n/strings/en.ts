export const en = {
  ...
  "state.quarantine.dm_title": ":warning: {count} quarantined in {source}",
  "state.quarantine.dm_entry": "• `{key}` — field `{field}`: {error}",
  "state.quarantine.dm_footer":
    "These entries were preserved (never deleted) and will NOT be used. Review them in the Home Tab under *Quarantined state*.",
  "state.quarantine.freeze_title":
    ":rotating_light: {source} paused — its state file could not be loaded",
  "state.quarantine.freeze_snapshot": "A snapshot of the file was saved to `{path}`.",
  ...
  "home.state_quarantine.hint":
    "These entries failed validation. They are preserved and never used. Retry re-validates; Delete removes permanently.",
  "home.state_quarantine.entry": "*{source}* · `{key}` — field `{field}`: {error}",
  "home.state_quarantine.retry": "Retry",
  "home.state_quarantine.delete": "Delete",
  "home.state_quarantine.freeze_banner":
    ":rotating_light: *State persistence paused* — {stores} could not be loaded and {count} store(s) are not being written. Repair the file(s) on disk to resume.",
