...
**Reason**: The `send_questions_instructions` MCP tool is removed. The full prompt (now `SEND_QUESTIONS_INSTRUCTIONS` constant in `src/plugins/trivia/scheduledPrompts.ts`) is baked into each game's `CronJobSpec.prompt` by `buildGameSpecs()` at plugin load time, with the `{game}` placeholder substituted to the game's `name`. There is no longer a runtime tool that returns the prompt on-demand.

**Migration**: Plugin-managed cron jobs created by `sdk.reconcileCronJobs("trivia", specs)` from `config.trivia.games[]` carry the substituted prompt directly. Operators do not invoke `create_schedules_instructions` to set up trivia anymore — they add entries to `config.trivia.games[]`.

### Requirement: Process Responses Instructions Tool

**Reason**: The `process_responses_instructions` MCP tool is removed for the same reason as `send_questions_instructions`. The full prompt (now derived from `getProcessResponsesInstructions(seasonsEnabled, game)` in `scheduledPrompts.ts`) is baked into each game's `CronJobSpec.prompt` for the reveal schedule.

**Migration**: Same as `send_questions_instructions` above.

### Requirement: Create Schedules Instructions Tool

**Reason**: The `create_schedules_instructions` MCP tool is removed. Trivia channels are now declared in `config.trivia.games[]`; the plugin reconciles cron jobs automatically on every load. Operators add entries to `config.trivia.games[]` (with `name`, `channel`, `questionCron`, `revealCron`, `timezone`, and optional `enabled`) and the cron jobs appear without admin tool interaction.

**Migration**: Operators add a new entry to `config.trivia.games[]` instead of asking Claude to call `create_schedules_instructions`. The existing migration `019-trivia-games-migration` automatically converts dispatcher-style legacy cron jobs into `config.trivia.games[]` entries on first boot post-upgrade.
...
### Requirement: Existing Trivia Cron Jobs Are Migrated

Migration `019-trivia-games-migration` SHALL detect dispatcher-style legacy `trivia` cron jobs (`prompt` matches "Call send_questions_instructions and follow" or "Call process_responses_instructions and follow"), pair them by channel, append `legacy-<channel>` entries to `config.trivia.games[]`, and delete the source jobs.

Inline fat-prompt legacy cron jobs (whose `prompt` does NOT match a dispatcher pattern) SHALL be left untouched by migration 019. Such jobs, on first scheduled fire post-upgrade, will fail at the first `mcp__trivia__*` tool call because the tool now requires a `game` argument the inline prompt does not pass. Operators SHALL delete and re-create these jobs by adding entries to `config.trivia.games[]`.

#### Scenario: Dispatcher-style legacy jobs are converted

- **GIVEN** `cron-jobs.json` contains one job with `plugin: "trivia"`, `prompt: "Call send_questions_instructions and follow..."` in channel `C123` AND one job with `plugin: "trivia"`, `prompt: "Call process_responses_instructions and follow..."` in the same channel
- **WHEN** migration 019 runs
- **THEN** `config.trivia.games[]` gains an entry `{ name: "legacy-C123", channel: "C123", questionCron: ..., revealCron: ..., timezone: ... }`
- **AND** both source jobs are removed from `cron-jobs.json`

#### Scenario: Pre-existing inline-prompt cron fails at first per-game tool call

- **WHEN** an inline-prompt cron job exists with `plugin: "trivia"` whose prompt does not match either dispatcher pattern
- **THEN** migration 019 leaves the job untouched
