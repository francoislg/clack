...
The system SHALL provide a `get_ideas` MCP tool (member role) that returns category suggestions for the next trivia question alongside server-computed hints that bias the question's truth value, difficulty, answer format, source type (fact vs topical), and lens, plus season-context fields that let the question-posting prompt detect the first fire of a new season and surface any human-readable season theme.
