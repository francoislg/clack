...
**Reason**: Moved to the `cascading-config-resolver` capability spec. The interpolation behavior is unchanged but is now specified as part of the resolver's responsibilities.
**Migration**: See `cascading-config-resolver` spec, "Variable Interpolation" requirement.
