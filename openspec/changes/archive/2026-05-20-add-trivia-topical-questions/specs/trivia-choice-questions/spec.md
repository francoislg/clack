...
- FROM: `### Requirement: Question type discriminator`
- TO: `### Requirement: Answers-format discriminator`

- FROM: `### Requirement: questionTypes is per-season, with config fallback`
- TO: `### Requirement: answersFormat is per-season, with config fallback`

- FROM: `### Requirement: Question-posting prompt branches on suggested type`
- TO: `### Requirement: Question-posting prompt branches on suggested answersFormat and questionType`

- FROM: `### Requirement: Bot auto-reactions sized to question type`
- TO: `### Requirement: Bot auto-reactions sized to answersFormat`
...
`answersFormat` resolution at `get_ideas` time SHALL follow this priority:

1. If the seasons feature is enabled AND `findCurrentSeason(state, Date.now())` returns a non-null `SeasonEntry` whose `format` is present AND the resolved slot (per the call's `slot` argument, default `0`) has an `answersFormat` field set, use that slot's `answersFormat`.
...
3. Otherwise (seasons disabled, `now` falls in a timeline gap, the current entry has no `format` or the slot has no `answersFormat`, AND the current entry has no top-level `answersFormat` field), use `config.trivia.answersFormat`.
...
#### Scenario: Current season's answersFormat overrides config (no format)
...
#### Scenario: Current season without answersFormat or format falls back to config
...
- **WHEN** `get_ideas` is called
- **THEN** the system uses `config.trivia.answersFormat` weights for the random roll
...
- **GIVEN** seasons are enabled with no current entry `answersFormat`, no format, AND `config.trivia.answersFormat` is absent
- **WHEN** `get_ideas` is called
- **THEN** the returned `suggestedAnswersFormat` is always `"boolean"`
...
- **WHEN** `get_ideas` reads the choice bounds (for a choice-formatted roll)
