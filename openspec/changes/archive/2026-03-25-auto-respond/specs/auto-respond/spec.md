...
- **THEN** the system triggers an response
...
- **THEN** the system does NOT trigger an response
...
- **THEN** the system does NOT trigger an response regardless of rules
...
- **THEN** the system uses a fallback message text such as "Respond this alert" for `messageText`
...
- **THEN** the prompt SHALL include a mode description indicating this is an automated response triggered by a channel alert
- **AND** the prompt SHALL instruct Claude that the message content is an alert or notification to respond, not a human question
...
- **AND** Claude uses this context to guide the response (e.g., "This is a Sentry error alert. Focus on the stack trace.")
...
- **AND** does NOT allow the error to propagate to `executeAndDeliver`'s internal `handleError()` which would post error blocks to the thread and attempt a DM to the synthetic user
- **AND** does NOT crash the event handler

#### Scenario: Streamer receives synthetic user ID
- **WHEN** the streaming response system receives the synthetic `"auto-respond"` user ID as `recipient_user_id`
- **THEN** the system SHALL pass `undefined` instead of the synthetic ID to avoid sending an invalid user ID to the Slack API
...
- **WHEN** a message matches an auto-respond rule and triggers an response
