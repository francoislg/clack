...
### Requirement: runClaude accepts external AbortController

**Reason:** The worker pipeline no longer accepts an external `AbortController`. Cancellation flows through `handle.stop(...)` on the `ClaudeRunHandle` returned by `executeChange`. The handle owns its `AbortController` internally; callers do not pass one in.

**Migration:** Replace any `runClaude({ ..., abortController })` call sites with `executeChange({...})`-returns-handle and call `handle.stop(...)` to cancel.
