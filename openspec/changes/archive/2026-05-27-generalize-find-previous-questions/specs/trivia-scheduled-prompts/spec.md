...
The prompt SHALL NOT instruct Claude to pass a `games` argument (or any legacy single-game arg) to `find_previous_questions` during duplicate detection. Duplicate detection is intentionally cross-game.

The prompt SHALL NOT contain any wording instructing Claude that duplicate detection is "game-scoped" or limited to the current game's history. The previous cross-cutting carve-out asserting game-scope SHALL be removed in favor of explicit cross-game wording at step 4.
