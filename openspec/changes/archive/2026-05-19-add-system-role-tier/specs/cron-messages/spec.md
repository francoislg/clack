...
- **AND** legacy jobs persisted with `createdBy: "<pluginName>"` and `pluginManaged: true` (pre-migration shape) are rewritten by the boot migration introduced in this change to `createdBy: null` + `systemActor: "plugin:<pluginName>"`
