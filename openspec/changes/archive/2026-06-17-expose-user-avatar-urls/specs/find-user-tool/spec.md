...
- **AND** returns all matching users as an array of `{ userId, username, displayName, avatarUrl }`
...
- **AND** each user entry includes `avatarUrl`, the member's public Slack profile picture URL
