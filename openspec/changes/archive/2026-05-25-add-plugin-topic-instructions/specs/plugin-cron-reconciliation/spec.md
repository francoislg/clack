...
interface CronJobSpec {
  ...
  attachedTopics?: string[]; // topic names auto-attached when this job fires (see plugin-topic-instructions)
}
```
...
When `spec.attachedTopics` is present, it SHALL be passed through to `createJob` (for new entries) and applied to the persisted `attachedTopics` field (for in-place updates) following the same resolution rules as other spec fields. When `spec.attachedTopics` is absent on re-reconcile of an existing job that had a value, the persisted `attachedTopics` SHALL be cleared (not preserved) so that a plugin can declaratively remove the field by omitting it.
...
#### Scenario: New spec with attachedTopics persists the array

- **GIVEN** no cron jobs exist with `plugin === "trivia"`
- **WHEN** `sdk.reconcileCronJobs("trivia", [{ specKey: "game-a:question", cronExpression: "0 9 * * 1-5", channel: "C123", prompt: "…", timezone: "UTC", attachedTopics: ["trivia"] }])` is called
...
#### Scenario: Re-reconcile with new attachedTopics overwrites the persisted array

- **GIVEN** a cron job exists with `plugin === "trivia"`, `specKey === "game-a:question"`, `attachedTopics: ["trivia"]`
...
#### Scenario: Re-reconcile without attachedTopics clears the persisted field

- **GIVEN** a cron job exists with `plugin === "trivia"`, `specKey === "game-a:question"`, `attachedTopics: ["trivia"]`
- **WHEN** the spec is re-reconciled with the same `specKey` but no `attachedTopics` field
- **THEN** the persisted job no longer has an `attachedTopics` field (or has an empty array)
...
- **AND** all other spec fields (`cronExpression`, `prompt`, `channel`, `timezone`, `name`, `requiredTools`, `skipConditions`, `attachedTopics`) are updated to the spec values
