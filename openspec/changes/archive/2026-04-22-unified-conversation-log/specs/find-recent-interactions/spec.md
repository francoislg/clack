...
The system SHALL provide a `find_recent_interactions` query tool that searches persisted Q&A session history so Claude can recover context from prior interactions. The tool returns a lightweight summary per session; full transcripts are retrieved separately via `find_session_transcript`.

#### Scenario: Returns sessions matching keywords
- **WHEN** Claude calls `find_recent_interactions` with a `keywords` parameter
- **THEN** the system returns only sessions where the keyword string appears (case-insensitive) in any `UserMessage.text`, any `AssistantMessage.payload.message`, or the rendered text of any `AssistantMessage.payload.blocks`
- **AND** keyword matching considers every message in the unified `messages` array, not only the first or last
...
- **THEN** only sessions with matching `triggerType` are returned
- **AND** valid values are `"reactions"`, `"directMessages"`, `"mentions"`, `"autoRespond"`, `"threadReply"`, or `"scheduled"`
...
- **THEN** each result includes: `sessionId`, `channelId`, `channelName`, `triggerType`, `userId`, `displayName`, `createdAt`, `lastActivity`, `firstQuestion` (text of the initial `UserMessage`), `latestAssistantText` (text derived from the most recent `AssistantMessage.payload`, or undefined if the latest assistant turn was skipped), `messageCount`, `assistantTurnCount`, and `skippedTurnCount`
- **AND** the result does NOT include full `payload`, `blocks`, `toolCalls`, or the complete `messages` array — callers fetch the full transcript via `find_session_transcript`
