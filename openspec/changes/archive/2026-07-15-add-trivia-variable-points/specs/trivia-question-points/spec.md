...
Variable per-question point values for trivia: a cascading `points` axis whose cap and guidance are admin-configured, whose concrete value is Claude-picked at generation time, stamped on the question record, displayed on the card, and paid out by every scoring surface.
