...
The system SHALL detect config update intent via the `propose_config_update` MCP tool call instead of XML tag parsing.
...
**Reason**: The `{CONFIG_UPDATE_BLOCK}` variable injected XML format documentation, file lists, and read paths into the admin system prompt. Replaced by: `propose_config_update` tool schema (self-documenting format), `list_config_files` query tool (dynamic file list), and Claude's existing `Read` tool (for reading current file content).

**Migration**: Remove `{CONFIG_UPDATE_BLOCK}` from admin instruction files. The tool schema and query tools provide equivalent information on demand.
