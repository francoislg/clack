...
- **WHEN** the trivia plugin's `registerTopic` call is inspected
...
- **WHEN** the trivia plugin loads
- **THEN** each of the seven config-mutation tools is registered via `sdk.registerTool("admin", ..., { topic: "trivia:management" })`
- **AND** none of the seven omits the `{ topic: "trivia:management" }` options object
