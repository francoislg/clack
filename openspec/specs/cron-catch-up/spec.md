# cron-catch-up Specification

## Purpose

Recover cron fires missed while the process was down (deploys included). The 60-second scheduler tick only matches slots in the last minute, so a slot passing during downtime is otherwise silently lost. This capability provides the boot-time detection layer (missed-run computation from persisted `lastRunAt`), the delayed-boot plugin hook that dispatches after a configurable settle delay, and the owner-scoped fire-now lever plugins use to catch up — with all catch-up judgement living in plugin code, never in core.
