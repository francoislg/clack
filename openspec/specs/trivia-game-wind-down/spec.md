...
An optional end-of-round disabling mechanism for trivia games. A game configured with `disableAfterRound: true` stops posting new questions when a seasonal or seasonless round concludes, halting automatic trivia generation until an admin re-enables it. The wind-down path integrates with both seasonal and seasonless game timelines, using a shared executor and idempotent recovery protocol to ensure a closed season or cleared board is never accidentally re-reopened.
