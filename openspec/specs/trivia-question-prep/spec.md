...
- **AND** the prep spec's `requiredTools` is `["mcp__trivia__get_ideas", "mcp__trivia__find_previous_questions", "mcp__trivia__save_question"]` — specifically does NOT include `mcp__trivia__post_questions`
...
- **AND** the question spec's `requiredTools` includes `mcp__trivia__post_questions` (today's behavior, unchanged)
...
### Requirement: Prep cron tool allowlist excludes post_questions

The cron spec emitted for `<name>:prep` SHALL declare a `requiredTools` list that includes `get_ideas`, `find_previous_questions`, and `save_question` but SHALL NOT include `post_questions`. The exclusion is a structural defense against accidental Slack posts from the prep run, complementing the channelless `submit_response` restriction.

#### Scenario: Prep tool list cannot post

- **GIVEN** the prep cron spec has been emitted
- **WHEN** the cron run begins and Claude is given the tool list from `requiredTools`
- **THEN** `mcp__trivia__post_questions` is NOT in the available tools
- **AND** if Claude attempts to call it, the SDK rejects the call (tool not declared as required)
