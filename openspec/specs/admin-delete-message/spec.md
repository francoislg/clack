# admin-delete-message Specification

## Purpose
Provides an `admin_delete_message` tool that allows admin users to delete a Slack message posted by Clack, identified by its permalink URL. Verifies ownership before deletion and surfaces clear errors for ephemeral messages, non-Clack messages, and channel membership issues.
