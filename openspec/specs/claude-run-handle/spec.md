...
`ClaudeRunHandle` is the public control surface for one in-progress multi-turn Claude run. Returned synchronously by `clackSession`, it lets callers push follow-up user messages with `sendUpdate`, abort via `stop`, observe `status`, and await the final response via `futureResponse`. Replaces the prior `AsyncIterable<SDKMessage>` return shape so Slack handlers can interact with a live run rather than just consume its messages.
...
### Requirement: Pending-Input Gate

The handle SHALL expose `hasPendingInput(): boolean` returning true when at least one `sendUpdate` push is queued but not yet consumed by the SDK. The `submit_response` MCP tool SHALL refuse to finalize the run while `hasPendingInput()` is true, so Claude is forced to read queued user follow-ups before the turn ends.

#### Scenario: hasPendingInput tracks queue depth

- **WHEN** `handle.sendUpdate(text)` succeeds
- **THEN** `handle.hasPendingInput()` returns `true` until the SDK reads that push from the input stream
- **AND** returns `false` once the SDK has consumed all queued pushes

#### Scenario: submit_response rejects while pending input exists

- **WHEN** Claude calls `submit_response` while `handle.hasPendingInput()` returns true
- **THEN** the tool returns an error result instructing Claude to read and address the queued message before resubmitting
- **AND** does NOT deliver a response to Slack
- **AND** the SDK turn loop continues, so the queued message surfaces as the next user turn
