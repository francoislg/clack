# repo-setup-memory Specification

## Purpose

Self-learning repo setup knowledge for worker and tester runs: one living memory entry per repo per run kind (`worker-setup:<repo>` / `tester-setup:<repo>`) that runs read at start (injected deterministically into their prompts), verify against the repository, and rewrite at end of run — converging toward a clean, current setup recipe with doc-source provenance, advisory under operator-authored instruction files.
