# trivia-reveal-in-cards

## Purpose

Support for persisting Claude-authored narrative commentary on revealed trivia question cards, gated by an `includeRevealInQuestions` axis. When enabled on a game or workspace, each revealed question card displays not just the deterministic facts footer (Answer / Correct / Incorrect / counts) but also a narrative block — the WHY, fun facts, "nobody cracked it" teaching — authored by Claude during the reveal process and persisted on the question record. The feature is disabled by default so existing games behave exactly as before.
## Requirements
