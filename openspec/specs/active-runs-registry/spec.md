...
The "consult registry then spawn fresh" decision used by Slack handlers SHALL be atomic enough that two concurrent invocations cannot both spawn a fresh run for the same `(channelId, threadTs)`.
...
- **THEN** at most one of them constructs a new `ClaudeRunHandle` (the registry's set-if-absent semantics serialize the construction)
