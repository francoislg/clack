...
The Trivia plugin SHALL expose a `list_games` MCP tool gated to the `member` role that returns the list of games from `config.trivia.games[]`. The tool SHALL accept one optional argument:
...
```
{
  games: Array<{
    ...
    channel: string,
    timezone: string,
    enabled: boolean         // defaults exposed as true when omitted from config
  }>,
  total: number
}
```

Entries SHALL be returned in their config order. The response SHALL NOT include `questionCron` or `revealCron` (those are scheduling details, not surfaced to Claude's planning surface).
