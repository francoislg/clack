...
Cancellation of in-flight Claude invocations — via message edits, the stop reaction, or the inline stop emoji. Built on top of the `active-runs-registry` capability, where each in-flight run is represented by a `ClaudeRunHandle` (per `claude-run-handle`) that owns its own `AbortController` and is keyed by `(channelId, threadTs)`.
