# trivia-question-contexts

## Purpose

Adds an optional `contexts` (lens) axis to trivia question generation. When configured at any cascade tier (slot → season → config), `get_ideas` returns a freshly-rolled weighted-random ordering of every configured lens (`contextPriority: string[]`). Claude tries lenses in priority order — using the first as the question's slant — and descends only when the current lens yields no usable question. The empty-string lens is a first-class "no specific lean" option. When `contexts` is absent at every tier, no `contextPriority` is returned and generation runs without a lens (pre-change behavior).
