...
An optional "Tell me more" button on a revealed trivia question's card. When enabled (game or workspace tier), clicking it removes the button, posts an intro tagging the clicker, and starts a thread conversation where Clack digs into the question and its answer — then the thread auto-follows for further questions.
...
A single action handler, registered once via a regex matching `tell-me-more:<questionId>` (SDK-scoped), SHALL serve the "Tell me more" button on every question. On click it SHALL acknowledge immediately, then: (1) remove the "Tell me more" button from the shared card via `chat.update` — dropping the dedicated tell-me-more actions block while retaining the reveal footer and the "See your answer" button — so the button is removed globally for all viewers; (2) post a short localized intro reply in the question's thread that tags the clicking user (e.g. *"User &lt;@U123&gt; asked for more information, here we go:"*); and (3) start a Claude Q&A turn in that thread (via the `plugin-thread-conversation` SDK capability) seeded with the question text and correct answer, instructing Claude to surface interesting details about the question and its answer.

The thread anchor SHALL be the question's own message (its `ts`), so the conversation hangs as a thread under the revealed question.
...
- **WHEN** the clicked `action_id` cannot be parsed to a known question, or the message lacks the blocks/anchor needed to update it
