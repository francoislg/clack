# admin-deference Specification

## Purpose
Make Clack defer to a verified admin (instead of re-litigating its prior assessment) when the admin explicitly invokes admin authority via a keyword in their latest message, and rebut the same claim when it comes from a non-admin — keeping the trust boundary structural (decided by the verified role, never by message text).
