...
2. Identify a newsworthy event from the search results, preferring events from the last day or two and going back no more than approximately one week.
3. Write a `boolean` or `choice` question (per `suggestedAnswersFormat`) anchored on the chosen event.
4. Capture the source URL (the most authoritative result that supports the claim) and pass it to `save_question` as `sourceUrl`.
5. Optionally capture the event date and pass it as `eventDate`.
6. Apply the same downstream gates as the fact paths (polarity self-check for boolean; distractor plausibility for choice; difficulty self-rating).

When WebSearch returns no usable result for the chosen lens, Claude SHALL descend the context priority list per `trivia-question-contexts`. When no lens yields a usable result, Claude SHALL re-call `get_ideas` to re-roll.
...
#### Scenario: Topical path falls back when no event found

- **GIVEN** WebSearch returns no usable results for the chosen category and context
- **WHEN** Claude has exhausted the context priority list
- **THEN** the prompt instructs Claude to re-call `get_ideas` to re-roll the question
