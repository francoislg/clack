# trivia-post-game-buttons

## Purpose

The buttons appended to a revealed trivia question's card (the "post-game buttons" — "See your answer", "Tell me more", and any future additions) are owned by a single declarative registry. The registry is the source of truth for which buttons exist; shared helpers render them as a contiguous section, install one action handler per entry at boot, and statically remove one-shot buttons on click while preserving their siblings. Adding a button is a single registry entry plus its click behavior — the renderer, installer, and remover never change.
