# pr-spinoff Specification

## Purpose
Let a worker, mid-run, carve a self-contained slice of its current changes into a SEPARATE pull request — handled as a standalone sibling change session with its own Slack thread, branch, and follow-up lifecycle — without the worker acquiring a second worker or mutating any existing session's branch. Preserves the 1 session : 1 branch : 1 PR invariant by producing N independent sessions.
...
- **THEN** the sibling fails gracefully in its own thread with a failure message, and the originating session and other siblings are unaffected
