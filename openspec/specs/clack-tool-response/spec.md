...
At the Slack send boundary the prepared table is rendered and appended to the `blocks` array passed to `chat.postMessage`:

- **Default variant** (`variant` absent or `"table"`): the legacy Slack `table` block, with `column_settings` alignment/wrap preserved — byte-for-byte as before this change.
- **`variant: "data_table"`**: a Slack `data_table` block (interactive pagination and sortable columns). String/`raw_text` cells map 1:1, `rich_text` cells pass through, a default `page_size` constant is applied, and the `caption` (authored, or a default label) is set. Slack's `data_table` has no per-column alignment, so any authored `column_settings` is dropped under this variant. When the estimated serialized size of the converted `data_table` exceeds Slack's 20,000-character aggregate cap, delivery SHALL fall back to the legacy `table` block for that message (with a log line) instead of failing.

The same rendering applies to `post_to` sibling tables. Claude selects the variant per use case; it never selects the raw Slack block type directly.
...
#### Scenario: explicit categories must cover every data-point label

- **WHEN** a series chart supplies `axis_config.categories` that omit a data-point label
- **THEN** the tool returns a validation error naming the missing label(s)
- **AND** omitting `axis_config.categories` entirely is accepted (categories are auto-derived at render)
