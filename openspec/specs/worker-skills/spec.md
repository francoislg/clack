# worker-skills Specification

## Purpose
Built-in, overridable, lazily-loaded procedure skills for the Changes Workflow worker. A worker skill is a `SKILL.md` (frontmatter `description` = always-in-context trigger, body = the procedure) resolved through the existing two-tier instruction chain (`configuration/` overrides `default_configuration/`) in a global and a per-repo scope. Resolved skills' triggers render into a `WORKER SKILLS` catalog in the worker execution prompt; bodies load on demand via a worker-mode `load_skill` tool. Ships a built-in `rebase` skill.
