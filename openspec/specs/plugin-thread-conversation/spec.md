# plugin-thread-conversation

## Purpose

A plugin-SDK primitive for starting a full Claude Q&A turn in a channel/thread — unlike the single-turn, tool-less `askClaude`, it routes through core's `processMessage` so the turn runs with the normal query toolset, creates a real session, uses the common chat streamer, and leaves the thread auto-following.
