...
### Trivia plugin: optional reveal reminders (`remindMissedPlayers`)

Opt-in nudge for players who haven't answered the current round. Two independent switches gate it: a per-game `remindMissedPlayers?: boolean` flag (game-tier, `upsert_game`) AND a per-user `revealReminders` personal preference (default OFF, set from the Home Tab settings modal — the trivia plugin registers it via `sdk.registerPreferences`, and it persists in the user-preferences `plugins.trivia` slice, NOT trivia's own data namespace). When a game has the flag on, `buildGameSpecs` emits a fourth channelless spec `<name>:reminder` whose cron is `deriveReminderCron(revealCron)` — the reveal cron shifted one hour earlier (returns null, so NO reminder spec, when the reveal hour is `0` or not a single integer). The fire drives `REMIND_UNPLAYED_INSTRUCTIONS`, which calls the cron-only `remind_unplayed` tool: audience = trivia players (`loadUsers()`) who answered none of the posted-but-unrevealed questions AND have `revealReminders` on (`sdk.preferences.get`); each is DM'd via `sdk.dmUser` with one Claude-composed message (no user is named), continuing past per-recipient failures. Catch-up never backfills a missed reminder (the walker enumerates only `:question`/`:lock`/`:reveal`).
