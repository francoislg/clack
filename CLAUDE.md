...
Optional plugin (`src/plugins/idler/`, gated by its own `config.json` `enabled` + a non-empty `repoAllowlist` + a `reportingChannel`). A channelless, cron-driven plugin modeled on `casual-talk` that turns idle time into reviewed, mergeable progress. The schedule is anchored by an explicit **`workHours`** window (`{ start, end, tz, days }`) that names WHEN the idler works — it fires INSIDE this window (set `start > end` for an overnight window, e.g. `18→9` = 6 PM–9 AM; any interior slice like `19→20` = 7–8 PM is expressible). Four cooperating cron tasks (up to), scheduled so sync and work never overlap. Sync is split into two tiers to keep token cost proportional to real change: **light sync** (`specKey: sync-light`, read-only — memory-triage ONLY, picking up newly-remembered work; fires at `:45` every **`syncEveryHours`** hours (default 2, settable via `set_idler_config`) across the sync window EXCEPT the anchor hour; when the recall page holds nothing new it ends immediately via `skip_response`, and its prompt omits the ~6k-token fetch-instructions doc since triage needs only the repo allowlist) and **deep sync** (`specKey: sync`, read-only — the full-maintenance pass: quick-fetch + close-resolved, coldest-unit re-verification/parking/priority-recompute, memory triage, and external discovery covering ALL enabled sources in one fire; runs ONCE per sync-window day at the **anchor hour** — the hour just before the work window opens, so it primes the ledger for the first work fire). The sync window is an explicit **`syncHours`** window or, when absent, the **complement** of `workHours`; the anchor is that window's last hour — `workHours.start − 1` for the complement (i.e. just before work opens), or `syncHours.end − 1` for an explicit window (whose last hour need not precede work). Then **work** (INSIDE `workHours` — advances one unit per fire along the `continue > triage > implement > review` ladder via `propose_change`+`auto`, query-mode triage/review, never merges), and **summary** (morning digest to `reportingChannel`, defaults to 9 AM via `summaryHour`). The work cadence is configurable via **`workEveryMinutes`** (settable via `set_idler_config`) — a divisor of 60 in `[5, 60]` (`5, 6, 10, 12, 15, 20, 30, 60`), **default 30** (was a hardcoded 15; set `workEveryMinutes: 15` to restore the old pace). Fewer fires ⇒ lower nightly token cost. `syncEveryHours` governs the LIGHT cadence only; the deep fire is always exactly one per window-day. The idler runs **only on the window's days** — on all other days (e.g. weekends) nothing fires (no work, no sync), so review work doesn't pile up unbounded for devs. The window math lives in `heuristic.ts` (`windowHours`/`complementHours`/`thinHours` + `syncSchedule`/`buildDeepSyncCron`/`buildLightSyncCron`/`buildWindowCron`); admins set the windows via `set_idler_work_hours` / `set_idler_sync_hours` (the latter takes `clear: true` to revert to the computed complement). A single-hour sync window reconciles only the deep fire (no light). The `data/plugins/idler/ideas.json` ledger holds minimal state (`open` + `priority` + `source` + free-text + growing self-describing `references[]` with read/comment recipes and idempotency cursors), keyed by stable source entity (e.g. an issue/ticket id) for dedup. Behavior/contract instructions ship as a topic; sourcing lives in admin-editable `data/plugins/idler/fetch-instructions.md`. Safety rails: repo allowlist, per-fire/per-night action caps, never-auto-merge.

### Split investigations

Optional feature gated by `config.investigations = { enabled, emoji }` (fail-fast zod, `emoji` default `"mag"` / 🔍, fully inert when disabled). A **split investigation** is a conversation re-home primitive that spins a question or discussion off a thread or DM into a dedicated **main surface** (an admin-configured investigations channel, or a DM with the requester) while following one or more origin threads read-only. Clack writes only to the main surface; followed threads are inputs, never outputs, except a one-time bootstrap breadcrumb.

The investigations channel is persisted in `data/state/investigations.json` (graceful zod), NOT in config — the same split as auto-respond, allowing admins to pick the channel from the Home Tab.

Three entry points funnel through `bootstrapInvestigation` (create main surface → follow origin → immediate first round over full origin history → breadcrumb): (1) the investigate emoji reaction on any message, (2) the `start_investigation` tool ("investigate on the side"), and (3) DM/channel relocation via `startInvestigationFrom`.

Two follow modes per origin thread: **`followAndInteract`** (each new side-thread message runs a subject-keyed pre-analysis classifier; a `respond` verdict drives a round) and **`follow`** (piggyback: `pendingCount` accrues, surfaced next round). Event-driven with no polling — a message-event tee matches the open-investigations index by `(channel, threadTs)` and is non-destructive (auto-respond/mentions unaffected). A per-thread `lastInjectedTs` cursor + drain-on-every-round makes downtime lossless; a boot reconciliation pass (`onDelayedBoot`) re-fires pending `followAndInteract` threads.
...
**Answer-ownership axis / shared buzzer (optional).** `answeringType?: "individual" | "byTeam"` cascades `season → game → workspace → "individual"` (structural-special like `teamsEnabled` — NOT a `CascadeAxes` member, no slot tier; resolved inside `resolveTeamsConfig`'s `EffectiveTeamsConfig`). `"individual"` (default) is the shipped model — every player owns their own `(userId, questionId)` answer, and the shipped `teamsScoring` aggregates individual answers at reveal. `"byTeam"` is a true shared buzzer: the **team** owns ONE answer slot per question (`data/plugins/trivia/games/<game>/team-answers.json`, keyed `(teamName, questionId)`), any member's click OVERWRITES the previous member's (last-click-wins, `lastAnsweredBy` retained for audit), the live roster/reveal show the team name, and non-roster players ("free agents") keep answering individually. It is EFFECTIVE only when the resolved teams config is enabled with a non-empty roster; otherwise it falls back to `"individual"` and surfaces an `inertAnsweringType` warning (the `inertEnabled` precedent). The seam is `AnsweringStrategy` (`answering/types.ts`): `IndividualAnswering` (legacy `(userId,questionId)` rows) and `ByTeamAnswering` (team slots + free-agent fallthrough, projecting slots to synthetic `SubmittedAnswer` rows with `userId: "team:<name>"`). `selectAnsweringStrategy(question, scoped, data)` is the ONE dispatch point — it reads the question's STAMP, never live config: `post_questions` stamps `answeringType` + the frozen roster (`teamsStamp: { teams }`) so a mid-round knob/roster edit never reshuffles a live question's slots (the `tagPlayers` pattern). Scoring is points-primary and slot-aware: `computeLeaderboard`/`computeRoundSummary` FILTER `team:` rows out of the individual surfaces (teams show only in team standings), while `computeTeamStandings` gains a slot path (one authoritative row per team per question, matched by name, summing with the aggregate path in a mixed season). Team writes NEVER touch identity plumbing (`recordJoin`/`refreshIdentities`) — a guard test (`answering/loadAnswersGuard.test.ts`) keeps scoring-view reads inside the strategy seam and the `byTeam` tests assert no `team:` key reaches the registry. Audit family: `save_cheating` sweeps a slot the flagged member holds (clean teammate can re-answer); `override_answer` accepts a `team:<name>` owner key; `get_question_history` surfaces team slots with `lastAnsweredBy` + the `ownerKey`; "See your answer" shows a member the team's slot (attribution suppressed). All rendering routes through `strategy.ownerLabel` (bold `*TeamName*`, never a mention). Surfaced by `list_games`/`list_seasons` (+ inert warning) when set; admin-edited via `upsert_game` / `upsert_season` / `set_workspace_config`. When unset/`"individual"` everywhere, behavior is byte-identical to pre-feature trivia.
...
### Trivia plugin: reveal reminders (per-user, per-game opt-in)

A nudge DMed to players who haven't answered the current round, one hour before reveal. The feature is **always on** at the schedule level — for every enabled game whose reveal hour is shiftable, `buildGameSpecs` emits a channelless `<name>:reminder` spec whose cron is `deriveReminderCron(revealCron)` (the reveal cron shifted one hour earlier; returns null, so NO reminder spec, when the reveal hour is `0` or not a single integer). There is NO admin gate — whether anyone is DMed is driven entirely by per-user opt-in.

Each user opts in **per game** from the Home Tab settings modal: the trivia plugin registers one toggle per enabled game via `sdk.registerPreferences` (field key `game:<name>` via `revealReminderKey`, label = the prettified game name, section header = the localized `prefs.reveal_reminders.title`). Registration happens after the config bridge warms (so the game list is known) and refreshes on the trivia soft-restart when games change. Values persist in the user-preferences `plugins.trivia` slice as a permissive `z.record(string, boolean)` — NOT trivia's own data namespace.

The reminder fire drives `REMIND_UNPLAYED_INSTRUCTIONS`, which calls the cron-only `remind_unplayed` tool: audience = trivia players (`loadUsers()`) who answered none of the posted-but-unrevealed questions AND have this game's key set to `true` (`sdk.preferences.get`); each is DM'd via `sdk.dmUser` with one Claude-composed message (no user is named), logging and continuing past per-recipient failures. Catch-up never backfills a missed reminder (the walker enumerates only `:question`/`:lock`/`:reveal`).
