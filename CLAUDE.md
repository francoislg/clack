...
- **No hand-rolled fakes.** A collaborator gets exactly one of two treatments: the canonical fake from the nearest `testHelpers.ts` (e.g. trivia's `createFakeSdk`, `createInMemoryDataLayer`) used as-is, or `vi.mock` at the boundary with interaction assertions (`toHaveBeenCalledWith` + assert the mock's return value is what got used/persisted). Never write a local stub that re-implements a dependency's behavior — it's a third implementation that drifts. Capture-style overrides (recording calls) should be `vi.fn()` instances asserted through the mock API, not closure-variable push arrays.
- **Fakes are open-closed.** If a canonical fake is wrong or missing behavior, fix it in `testHelpers.ts` so every consumer benefits; if one test needs a variant, pass overrides (`createFakeSdk({ ... })`). Locally shadowing or re-implementing a fake is never an option. `sdk.t` / `sdk.actionId` have exactly one faithful rendering and must never be overridden at all (guard-enforced by `src/plugins/trivia/testHelpers.guard.test.ts`).
