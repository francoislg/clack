...
Optional plugin (`src/plugins/idler/`, gated by its own `config.json` `enabled` + a non-empty `repoAllowlist` + a `reportingChannel`). A channelless, cron-driven plugin modeled on `casual-talk` that turns idle time into reviewed, mergeable progress. Three cooperating cron tasks, scheduled so sync and work never overlap: **sync** (hourly, read-only — discovers/refreshes a work-unit ledger and recomputes priority; runs **during** active hours at `:45`, so the last fire before the active window closes primes the ledger for the first work fire), **work** (~15 min, **off-hours** — advances one unit per fire along the `continue > triage > implement > review` ladder via `propose_change`+`auto`, query-mode triage/review, never merges), and **summary** (morning digest to `reportingChannel`). The idler runs **only on active days** — on non-active days (e.g. weekends) nothing fires (no work, no sync), so review work doesn't pile up unbounded for devs. The `data/plugins/idler/ideas.json` ledger holds minimal state (`open` + `priority` + `source` + free-text + growing self-describing `references[]` with read/comment recipes and idempotency cursors), keyed by stable source entity (e.g. an issue/ticket id) for dedup. Behavior/contract instructions ship as a topic; sourcing lives in admin-editable `data/plugins/idler/fetch-instructions.md`. Safety rails: repo allowlist, per-fire/per-night action caps, never-auto-merge.
